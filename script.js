...
function produtos(nProd){
    ...
    document.getElementById("main").innerHTML += '<div id="avaliacao"><div id="titulo-avaliacao"><h4 id="avaliacao">AVALIAÇÕES</h4></div></div><div id="titulo-comentarios"><img id="icone-avaliacao" src="img/icone-avaliacao.png" alt="" /><h5>TODOS OS COMENTÁRIOS</h5></div><div class="linhaa"></div><div id="central"><div id="caixa-comentarios">'+'<div id="nomeUser"><div id="comentarioBox"></div>'+'<div id="funcoes-comentario"><button id="btn-comentar"><img id="icon-escrever" src="img/escrever.png" alt="" /></button><button id="btn-comentarios"><img id="icon-seta-btn" src="img/seta-btn.png" alt="" /></button></div></div></div></div></div>'

    dados(nProd)
}

var nomes = ["ANA", "MARIA", "DUDA", "ELIZA", "DIANA", "CATERINE"]
var estrelas = [3, 4, 4, 5, 5, 4]
var textos = ["Peça maravilhosa!", "A qualidade do produto me impressionou.", "Gostei muito do produto! Tamanho fiel a foto", "Tecido agradável", "Muito lindo! Adorei", "Recomendo esse produto!"]
...
function mostrarComentarios(){
    ...
    .then(corpo => { 
        //var tresComentarios = [corpo.vestido2.comentarios.comentario1, corpo.vestido2.comentarios.comentario2, corpo.vestido2.comentarios.comentario3]
        for(b=0; b<3; b++){
            var numAleatorio = Math.floor(Math.random() * nomes.length);
            document.getElementById("comentarioBox").innerHTML += "<div id='divNome'><p>"+nomes[numAleatorio]+"</p></div>"
            for(a=0; a<estrelas[numAleatorio]; a++){
                document.getElementById("comentarioBox").innerHTML +="<img src=imagens/estrelaCheia.png width='3%'>"
            }
            document.getElementById("comentarioBox").innerHTML += "<div id='nota'><p>"+estrelas[numAleatorio]+"</p></div>"
            document.getElementById("comentarioBox").innerHTML += "<div id='comentario'><p>"+textos[numAleatorio]+"</p></div>"
